...
import requests
from mutagen import flac as mutagen_flac
...
def captchaSent(phone: int, ctcode: int, api: str) -> bool:
    ...
    data = {'phone': str(phone), 'ctcode': str(ctcode)}
    data = requests.post(url, data=data).json()
    if data['code'] == 200:
        return True
    else:
...
def captchaCheck(phone: int, captcha: int, ctcode: int, api: str) -> bool:
    ...
    data = {'phone': str(phone), 'captcha': str(captcha), 'ctcode': str(ctcode)}
    data = requests.post(url, data=data).json()
    if data['code'] == 503:
        return False
    elif data['code'] == 200:
...
def captchaLogin(phone: int, captcha: int, ctcode: int, api: str) -> dict:
    ...
    data = {'phone': str(phone), 'captcha': str(captcha), 'countrycode': str(ctcode)}
    data = requests.post(url, data=data).json()
    if data['code'] == 200:
        return data
    else:
...
def passwordLogin(phone: int, password: str, ctcode: int, api: str) -> dict:
    url = api + '/login/cellphone?timestamp=' + timestamp()
    data = {'phone': str(phone), 'md5_password': md5(password.encode('utf-8')).hexdigest(), 'countrycode': str(ctcode)}
    data = requests.post(url, data=data).json()
    if data['code'] == 200:
        return data
    else:
...
def getSongInfo(songlist: list, api: str) -> dict:
    ...
    data = {'ids': ids.rstrip(','), 'cookie': COOKIE}
    data = requests.post(url, data=data).json()
    if data['code'] == 200:
        return data
    else:
...
def getSongUrl(songlist: list, api: str) -> dict:
    ...
    data = {'id': id.rstrip(','), 'cookie': COOKIE, 'level': 'jymaster'}
    data = requests.post(url, data=data).json()
    if data['code'] == 200:
        return data
    else:
...
class SongFile:  # TODO
    ...
    def setTag(self) -> bool:
        ...
        filetype = ospath.splitext(self.filePath)[1].lower()
        title = self.songTag['title']
        album = self.songTag['album']
        artists = []
        for i in self.songTag['artist']:
            artists.append(i['name'])
        if filetype == '.flac':
            audiofile = mutagen_flac.FLAC(self.filePath)
            audiofile['title'] = title
            audiofile['artist'] = artists
            audiofile['album'] = album
            if ('lyric' in self.songTag) and (self.songTag['lyric'] != None):
                audiofile['lyrics'] = self.songTag['lyric']
            if ('picture' in self.songTag) and (self.songTag['picture'] != None):
                response = requests.get(self.songTag['picture'])
